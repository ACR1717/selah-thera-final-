...
function App() {
  ...
  const [preselectedService, setPreselectedService] = useState("Valoración Inicial");
  ...
  const [isLegalOpen, setIsLegalOpen] = useState(null);
  ...
  const openWhatsApp = (msg) => window.open(`https://wa.me/50650111177?text=${encodeURIComponent(msg)}`, '_blank');
  ...
  const heroSlides = [
    { image: "https://i.postimg.cc/sxPRCZ4Z/grounding.jpg", title: "Mi camino hacia la salud integral empieza aquí" },
    { image: "https://i.postimg.cc/DZKjkpjQ/helioterapia.jpg", title: "Renuevo mi cuerpo y mente con terapias naturales" },
    { image: "https://i.postimg.cc/BQtdCydv/vitamin-b12-4796944-1280.jpg", title: "Medicina ancestral + tecnología moderna = bienestar" },
    { image: "https://i.postimg.cc/CLfqkMLn/estres-(2).jpg", title: "Selah Thera House no es una clínica, es el lugar donde mi cuerpo recordó cómo sanarse a sí mismo" }
  ];
  ...
  const handleLeadSubmit = (e, type) => {
    e.preventDefault();
    const form = e.target;
    const name = form.elements.namedItem('name').value;
    const symptom = form.elements.namedItem('symptom').value;
    const date = form.elements.namedItem('date').value;
    const companion = form.elements.namedItem('companion').value;
    const therapy = form.elements.namedItem('therapy').value;
  ...
  const blogPosts = [
    ...
    },
    {
      ...
      image: "https://i.postimg.cc/pLx7Nx6M/ondas-escalares-ok.jpg",
  ...
  return (
    <div className="font-sans text-stone-800">
      ...
      <section className="relative min-h-screen flex items-center pt-56 md:pt-72 overflow-hidden bg-brand-dark">
        ...
        <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex flex-col md:flex-row items-center gap-12">
          ...
          <div className="md:w-1/2 space-y-8 animate-fade-in text-center md:text-left">
            ...
            {/* BUTTONS - NATIVE HTML IMPLEMENTATION FOR 100% SIZE GUARANTEE */}
            <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start mt-6 items-start">
               <div className="flex flex-col items-center sm:items-start gap-1 w-full sm:w-auto">
                  <button 
                    onClick={() => handleBookingOpen("Valoración Inicial")}
                    className="rounded-lg font-bold text-lg uppercase tracking-wide flex items-center justify-center gap-2 transition-all duration-300 transform hover:-translate-y-1 shadow-lg bg-[#D4AF37] text-white hover:bg-[#B5952F]"
                    style={{ minWidth: '280px', height: '60px' }}
                  >
                    Agendar Valoración Gratis <ArrowRight className="w-5 h-5" />
                  </button>
              ...
              <button 
                  ...
                  className="rounded-lg font-bold text-lg uppercase tracking-wide flex items-center justify-center gap-2 transition-all duration-300 transform hover:-translate-y-1 shadow-lg bg-[#1c2e2a] text-white hover:bg-white hover:text-[#1c2e2a]"
                  style={{ minWidth: '280px', height: '60px' }}
              >
                  Ver Planes
              </button>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            <div className="relative rounded-2xl overflow-hidden shadow-2xl group cursor-pointer" onClick={() => setIsVideoOpen(true)}>
               ...
               <img 
                 src="https://images.unsplash.com/photo-1544367563-12123d8965cd?auto=format&fit=crop&q=80&w=1000" 
                 alt="Selah Thera House Ambiente" 
      ...
      {/* --- SERVICES STEPS --- */}
      <section id="servicios" className="py-24 bg-[#E3E0D3]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12 text-center">
            ...
            <div className="flex flex-col items-center justify-center">
              <h3 className="text-white text-xl font-bold mb-4 tracking-wider">SELAH THERA HOUSE</h3>
              <p className="text-sm leading-relaxed max-w-xs mx-auto">
                ...
                Ayudamos a las personas a recuperar su salud sin fármacos invasivos.
              </p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center border-t lg:border-t-0 lg:border-l border-white/10 pt-8 lg:pt-0">
              ...
              <div className="mt-6">
                <Button 
                  variant="outline" 
                  className="!py-2 !px-4 !text-sm !border-brand-accent !text-brand-accent hover:!bg-brand-accent hover:!text-brand-dark"
          ...
          <div className="border-t border-white/5 mt-12 pt-8 text-center text-xs text-stone-600 flex flex-col sm:flex-row justify-center gap-4">
            <p>&copy; {new Date().getFullYear()} Selah Thera House. Todos los derechos reservados.</p>
            <div className="flex gap-4 justify-center">
               <button onClick={() => setIsLegalOpen('privacy')} className="hover:text-brand-accent transition-colors">Política de Privacidad</button>
               <button onClick={() => setIsLegalOpen('terms')} className="hover:text-brand-accent transition-colors">Términos de Uso</button>
               <button onClick={() => setIsLegalOpen('medical')} className="hover:text-brand-accent transition-colors">Descargo Médico</button>
            </div>
          </div>
        </div>
      </footer>
...
export default SelahTheraFinalRelease;
